...
async function testTranscriptProcessing() {
  ...
  try {
    ...
    console.log('\n🔍 Testing RAG with specific query about John Smith...');
    
    const specificRagResponse = await axios.post('http://localhost:3000/api/vapi-rag', {
      message: 'What did John Smith want to do with his business?',
      userId: 'test-user',
      callStage: 1,
      includeTranscripts: true,
      includeKnowledge: true,
    ...
    console.log('\n🎯 Specific RAG Response:', JSON.stringify(specificRagResponse.data, null, 2));
