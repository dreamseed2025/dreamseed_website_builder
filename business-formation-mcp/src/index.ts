...
const supabase = createClient(
  process.env.SUPABASE_URL || 'https://plmmudazcsiksgmgphte.supabase.co',
  ...
  try {
    switch (name) {
      case "extract_and_save_business_data":
        return await extractAndSaveBusinessData(
          (args as any)?.conversation_text, 
          (args as any)?.call_stage, 
          (args as any)?.customer_phone
        );

      case "get_customer_progress":
        return await getCustomerProgress((args as any)?.customer_phone, (args as any)?.customer_email);

      case "get_state_requirements":
        return await getStateRequirements((args as any)?.state);

      case "validate_business_name":
        return await validateBusinessName((args as any)?.business_name, (args as any)?.state);

      case "calculate_formation_costs":
        return await calculateFormationCosts((args as any)?.state, (args as any)?.entity_type, (args as any)?.package_type);

      case "get_formation_analytics":
        return await getFormationAnalytics((args as any)?.metric_type, (args as any)?.date_range);
...
async function extractAndSaveBusinessData(conversationText: string, callStage: number, customerPhone?: string) {
...
async function getCustomerProgress(customerPhone?: string, customerEmail?: string) {
...
async function getStateRequirements(state: string) {
  const requirements = (STATE_REQUIREMENTS as any)[state];
    ...
    content: [
      {
        ...
        text: `📋 ${state} LLC Formation Requirements:\n\n` +
               ...
               `Required Documents:\n${requirements.requirements.map((req: any) => `• ${req}`).join('\n')}`
      }
    ]
  };
}

async function validateBusinessName(businessName: string, state: string) {
...
async function calculateFormationCosts(state: string, entityType = 'LLC', packageType = 'Launch Basic') {
  const stateReqs = (STATE_REQUIREMENTS as any)[state];
  
  if (!stateReqs) {
    throw new Error(`State requirements not available for ${state}`);
  }

  const packageCosts: any = {
    'Launch Basic': 999,
    'Launch Pro': 1799, 
  ...
  try {
    switch (metricType) {
      case 'overview':
        ...
        const states = [...new Set(overview?.map((f: any) => f.state_of_operation).filter(Boolean))];
...
// Helper functions
function extractDataFromText(text: string, callStage: number) {
  const data: any = {
  ...
  if (phoneMatch) data.customer_phone = phoneMatch[1];

  // Extract business info
  const businessMatch = text.match(/(?:business|company|startup|venture)(?:\s+(?:is|called|named))?\s+([^.!?]+)/i);
  if (businessMatch) data.business_description = businessMatch[1];
  ...
  if (entityMatch) data.entity_type = entityMatch[1];
  else data.entity_type = 'LLC'; // default

  return data;
}

function calculateProgress(formationData: any) {
